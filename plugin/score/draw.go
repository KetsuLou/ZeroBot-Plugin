...
func drawScore16(a *scdata) (image.Image, error) {
	...
	canvas.DrawStringAnchored("杀币 + "+strconv.Itoa(a.inc), 350, 350, 0, 0)
	canvas.DrawStringAnchored("当前杀币："+strconv.Itoa(a.score), 350, 400, 0, 0)
...
func drawScore15(a *scdata) (image.Image, error) {
	...
	canvas.DrawString(a.nickname+fmt.Sprintf(" 杀币+%d", a.inc), float64(back.Bounds().Size().X)*0.1, float64(back.Bounds().Size().Y)*1.3)
	canvas.DrawString("当前杀币:"+strconv.FormatInt(int64(a.score), 10), float64(back.Bounds().Size().X)*0.1, float64(back.Bounds().Size().Y)*1.4)
...
func drawScore17(a *scdata) (image.Image, error) {
	...
	canvas.DrawStringAnchored("杀币 + "+strconv.Itoa(a.inc), 40, float64(imgDY-90), 0, 0)
	canvas.DrawStringAnchored("当前杀币："+strconv.Itoa(a.score), 40, float64(imgDY-60), 0, 0)
...
func customtext(a *scdata, fontdata []byte, cw, ch, aw float64, textcolor color.Color) (img image.Image, err error) {
	...
	canvas.DrawStringAnchored("杀币 + "+strconv.Itoa(a.inc), ((cw-scw)-(cw/3-scw/2))/8, (ch-sch)/2+sch/4+tempfh, 0, 0.5)
	...
	canvas.DrawStringAnchored("你有 "+strconv.Itoa(a.score)+" 枚杀币", ((cw-scw)-(cw/3-scw/2))/8, (ch-sch)/2+sch/4*3, 0, 0.5)
