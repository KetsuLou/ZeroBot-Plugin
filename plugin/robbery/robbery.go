...
func init() {
	...
	// 打劫功能
	engine.OnRegex(`^(打劫|狼袭)\s?(\[CQ:at,qq=(\d+)\]|(\d+))`, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			uid := ctx.Event.UserID
			fiancee := ctx.State["regex_matched"].([]string)
			victimID, _ := strconv.ParseInt(fiancee[3]+fiancee[4], 10, 64)
			if victimID == uid {
			...
			ctx.SendChain(message.At(uid), message.Text("打劫成功，钱包增加：", userIncrMonry, "杀币"))
			ctx.SendChain(message.At(victimID), message.Text("保险公司对您进行了赔付，您实际损失：", victimDecrMonry, "杀币"))
		})
}
